...
const ActivityRightPanel = ({
  ...
  return (
    <motion.div
      ...
      className={`w-72 xl:w-80 space-y-3 ${className}`}
    >
      {/* Header */}
      <div className="bg-gradient-to-r from-white/90 to-white/80 backdrop-blur-lg rounded-2xl p-3 shadow-xl border border-white/40">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-gray-800">
            {showActions ? 'Actions' : 'Activities'}
          </h3>
        </div>
      </div>
        ...
        actionItems.map((action, index) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`h-10 w-10 rounded-xl bg-gradient-to-r ${action.color} flex items-center justify-center shadow-md transition-all duration-300`}>
