...
import { ZUTCHI_ABI, ZUTCHI_CONTRACT_ADDRESS, ZIRCUIT_CONFIG } from './zutchiAbi';
...
export class ZutchiService {
  ...
  private getWalletClient() {
    ...
    console.log('ZutchiService: Wallet object properties:', Object.keys(this.wallet));
    ...
    if (this.wallet.readContract && this.wallet.writeContract) {
      console.log('ZutchiService: Found readContract/writeContract methods directly on wallet');
      return this.wallet;
    }
    ...
    if (this.wallet.connectorType === 'injected' && typeof window !== 'undefined') {
      console.log('ZutchiService: Trying to access window.ethereum for injected wallet');
    ...
    if (this.wallet.request) {
      console.log('ZutchiService: Found request method on wallet - using as ethereum provider');
      return this.wallet;
    }
    ...
    for (const key of Object.keys(this.wallet)) {
      ...
      if (value && typeof value === 'object' && value.request) {
        console.log(`ZutchiService: Found potential provider in property: ${key}`);
        return value;
      }
    }
    
    console.warn('ZutchiService: Could not find suitable wallet client. Available properties:', Object.keys(this.wallet));
    return null;
  }
  ...
  private getContract() {
    ...
    if (!this.walletClient.readContract && !this.walletClient.request) {
      console.warn('Wallet client does not have readContract method or request method. Available methods:', Object.keys(this.walletClient));
      return null;
    }
  ...
  private async readContract(functionName: string, args: any[] = []): Promise<any> {
    ...
    try {
      ...
      if (this.walletClient.request) {
        const data = this.contractInterface.encodeFunctionData(functionName, args);
        ...
        // Decode the result
        return this.contractInterface.decodeFunctionResult(functionName, result)[0];
      }
  ...
  private async writeContract(functionName: string, args: any[] = [], value: bigint = BigInt(0)): Promise<string> {
    if (!this.walletClient) throw new Error('No wallet client available');
    ...
    try {
      ...
      if (this.walletClient.request) {
        const data = this.contractInterface.encodeFunctionData(functionName, args);
        const accounts = await this.walletClient.request({ method: 'eth_requestAccounts' });
  ...
  async getZutchiTokenId(userAddress: string): Promise<bigint | null> {
    try {
      ...
      // Get the first token ID (this is a simplified approach)
      // You might need to implement a different method to get specific token IDs
      return BigInt(0); // Placeholder - implement proper token ID retrieval
    } catch (error) {
  ...
  async getZutchiAttributes(tokenId: bigint): Promise<ZutchiData | null> {
    try {
      ...
      const attributes = await this.readContract('getZutchiAttributes', [tokenId]);
  ...
  async feedZutchi(tokenId: bigint, foodAmount: bigint): Promise<boolean> {
    try {
      ...
      const hash = await this.writeContract('feedZutchi', [tokenId, foodAmount]);
  ...
  async putToSleep(tokenId: bigint, sleepDuration: bigint): Promise<boolean> {
    try {
      ...
      const hash = await this.writeContract('putToSleep', [tokenId, sleepDuration]);
  ...
  async putToWork(tokenId: bigint, workDuration: bigint): Promise<boolean> {
    try {
      ...
      const hash = await this.writeContract('putToWork', [tokenId, workDuration]);
  ...
  async getPotentialFrens(tokenId: bigint): Promise<bigint[]> {
    try {
      ...
      const potentialFrens = await this.readContract('getPotentialFrens', [tokenId]);
